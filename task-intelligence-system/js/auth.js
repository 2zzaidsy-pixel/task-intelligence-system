...
async function handleRegister(e) {
  ...
  try {
    ...
    // SECURITY: Always set role='user' from client.
    // Admin role is assigned server-side via database trigger
    // that checks the email against ADMIN_EMAIL.
    // This prevents client-side privilege escalation.
    if (data?.user) {
      const username = email.split('@')[0];
      await supabaseClient.from('profiles').upsert({
        id: data.user.id, email, username, role: 'user'
      });
    }
