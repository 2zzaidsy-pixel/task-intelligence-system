...
async function toggleDailyTask(id, currentStatus) {
  try {
    ...
    const { error } = await supabaseClient
      .from('daily_tasks')
      .update({ completed: completing, completed_at: now })
      .eq('id', id);

    if (error) throw error;
    showToast(completing ? '🎉 تم إكمال المهمة! +10 XP' : 'تم إلغاء إكمال المهمة', completing ? 'success' : 'warning');

    if (completing) {
      try {
        if (typeof addXp === 'function') {
          await addXp(currentUser.id, 10, 'daily_task', id, 'إكمال مهمة يومية');
          await incrementCompletedQuests(currentUser.id);
